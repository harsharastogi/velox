...
def bias(args):
    ...
    function_bias_output = f"{output_dir}/bias_functions"
    if status:
        return status

    if bias_output:
        with open(function_bias_output, "w") as f:
...
def bias_signatures(base_signatures, contender_signatures, tickets):
    """Returns newly added functions.
    Contents are of the form : `fn_name1=10,fn_name2=10`.
    Returns 1 if it detects incompatible changes.
    """
...
def parse_args(args):
    ...
    bias_command_parser.add_argument("output_dir", type=dir_path)
    bias_command_parser.add_argument("ticket_value", type=get_tickets)
